...
function addComboToCart(combo, addToCart) {
  ...
  combo.items.forEach(item => {
    ...
    addToCart({ id: `combo-${combo.id}-${item.name}`, brand: item.brand, name: item.name, isPartial: false }, item.size, p);
  });
}
...
export default function Home() {
  ...
  const [tab, setTab]             = useState('home');  // home | brands | partials
  const [brandCat, setBrandCat]   = useState('niche'); // sub-tab inside brands
  ...
  // Brands by category
  const brandsInCat = [...new Set(products.filter(p => p.category === brandCat).map(p => p.brand))].sort();

  // Products for selected brand
  const brandProducts = selectedBrand ? products.filter(p => p.brand === selectedBrand) : [];

  // Search results
  const searchProducts = products.filter(p => `${p.brand} ${p.name} ${p.notes || ''}`.toLowerCase().includes(searchQ));
  const searchPartials = visiblePartials.filter(p => `${p.brand} ${p.name} ${p.notes || ''}`.toLowerCase().includes(searchQ));

  const switchTab = (id) => { setTab(id); setSelectedBrand(null); };

  const S = {
    subTab: (active) => ({
      background: 'none', border: 'none', cursor: 'pointer',
      fontFamily: 'var(--ff-sans)', fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase',
      padding: '8px 16px',
  ...
  return (
    <>
      <Nav
        ...
        onSearch={(v) => { setSearch(v); if (v) setTab('home'); }}
        searchValue={search}
        activeTab={tab}
      ...
      {/* ── SEARCH RESULTS ── */}
      {searchQ && (
        <main style={{ maxWidth: 1400, margin: '0 auto', padding: '2rem 4vw 6rem' }}>
          <p style={{ fontSize: 12, color: 'var(--t3)', marginBottom: 20 }}>
            {searchProducts.length + searchPartials.length} result{(searchProducts.length + searchPartials.length) !== 1 ? 's' : ''} for "<span style={{ color: 'var(--t1)' }}>{search}</span>"
          </p>
          {searchPartials.length > 0 && (
            <div style={{ marginBottom: 32 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(220px,1fr))', gap: 12 }}>
                {searchPartials.map(p => <PartialCard key={p.id} partial={p} onAdd={addToCart} />)}
              </div>
            </div>
          )}
          {searchProducts.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 12 }}>
              {searchProducts.map(p => <ProductCard key={p.id} product={p} onAdd={addToCart} />)}
            </div>
          )}
      ...
      {!searchQ && tab === 'home' && (
        <>
            ...
            <div style={{ position: 'relative', zIndex: 1, maxWidth: 1400, margin: '0 auto', padding: '3rem 4vw', width: '100%' }}>
              <div style={{ maxWidth: 580 }}>
                ...
                <h1 style={{ fontFamily: 'var(--ff-serif)', fontSize: 'clamp(2.4rem,5vw,4rem)', fontWeight: 400, color: 'rgba(255,255,255,0.95)', lineHeight: 1.12, marginBottom: 18 }}>
                  Scent is the one<br />luxury <span style={{ color: 'var(--gold)' }}>everyone</span><br />deserves.
                </h1>
                ...
                <div style={{ display: 'flex', gap: 10 }}>
                  <button onClick={() => switchTab('brands')} style={{ padding: '12px 24px', borderRadius: 4, background: '#b09060', border: 'none', color: '#fff', fontFamily: 'var(--ff-sans)', fontSize: 12, fontWeight: 500, letterSpacing: '0.12em', textTransform: 'uppercase', cursor: 'pointer' }}>
                    Shop Now
                  </button>
          ...
          {/* Combos */}
          <main style={{ maxWidth: 1400, margin: '0 auto', padding: '3rem 4vw 6rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(300px,1fr))', gap: 16, marginBottom: '4rem' }}>
              {COMBOS.map(combo => {
                  ...
                  >
                    ...
                    <div style={{ borderTop: '0.5px solid var(--border)', paddingTop: '0.75rem', marginBottom: '1rem' }}>
                      {combo.items.map((item, i) => (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'rgba(255,255,255,0.45)', padding: '4px 0' }}>
          ...
          {selectedBrand ? (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
                <button onClick={() => setSelectedBrand(null)} style={{ background: 'none', border: '0.5px solid var(--border)', borderRadius: 4, padding: '5px 12px', color: 'var(--t3)', cursor: 'pointer', fontSize: 12, fontFamily: 'var(--ff-sans)' }}>← All Brands</button>
                <div>
                  <h2 style={{ fontFamily: 'var(--ff-serif)', fontSize: '1.4rem', color: 'var(--t1)', fontWeight: 400 }}>{selectedBrand}</h2>
                  <p style={{ fontSize: 11, color: 'var(--t3)' }}>{brandProducts.length} fragrance{brandProducts.length !== 1 ? 's' : ''}</p>
                </div>
              </div>
              {loading ? (
                <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--t3)' }}>Loading...</div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(200px,1fr))', gap: 12 }}>
                  {brandProducts.map(p => <ProductCard key={p.id} product={p} onAdd={addToCart} />)}
                </div>
              )}
            </>
          ) : (
            <>
              ...
              <div style={{ display: 'flex', gap: 0, borderBottom: '0.5px solid var(--border)', marginBottom: 24 }}>
                {BRAND_CATS.map(cat => (
                  <button key={cat.id} onClick={() => setBrandCat(cat.id)} style={S.subTab(brandCat === cat.id)}>
                    {cat.label}
                  </button>
                ))}
              </div>
              ...
              {loading ? (
                ...
                <div style={{ textAlign: 'center', padding: '4rem', color: 'var(--t3)' }}>No brands in this category yet.</div>
              ) : (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(150px,1fr))', gap: 10 }}>
                  {brandsInCat.map(brand => {
                    const count = products.filter(p => p.brand === brand && p.category === brandCat).length;
                    return (
                      <button key={brand} onClick={() => setSelectedBrand(brand)} style={{ background: 'rgba(255,255,255,0.02)', border: '0.5px solid var(--border)', borderRadius: 8, padding: '18px 14px', cursor: 'pointer', textAlign: 'left', fontFamily: 'var(--ff-sans)', transition: 'all .18s' }}
                        onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(176,144,96,0.5)'; e.currentTarget.style.background = 'rgba(176,144,96,0.07)'; e.currentTarget.style.transform = 'translateY(-2px)'; }}
                      ...
                      >
                        <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.85)', fontWeight: 500, marginBottom: 6, lineHeight: 1.3 }}>{brand}</div>
      ...
      {!searchQ && tab === 'partials' && (
        <main style={{ maxWidth: 1400, margin: '0 auto', padding: '2rem 4vw 6rem' }}>
          {loading ? (
            ...
            <div style={{ textAlign: 'center', padding: '6rem', border: '0.5px solid var(--border)', borderRadius: 12 }}>
              <div style={{ fontSize: 40, marginBottom: 16 }}>🧴</div>
              <p style={{ fontFamily: 'var(--ff-serif)', fontSize: '1.2rem', color: 'var(--t2)', marginBottom: 8 }}>No partials available right now</p>
              <p style={{ fontSize: 13, color: 'var(--t3)' }}>
                Follow <a href="https://www.instagram.com/the_scent_snob_/" target="_blank" rel="noopener noreferrer" style={{ color: 'var(--gold)' }}>@the_scent_snob_</a> for drop announcements
              </p>
            </div>
          ) : (
            <>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 24 }}>
                ...
                <span style={{ fontSize: 12, color: 'var(--t3)', flexShrink: 0 }}>{visiblePartials.length} available</span>
              </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              <a href="https://www.instagram.com/the_scent_snob_/" target="_blank" rel="noopener noreferrer" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, color: 'rgba(255,255,255,0.65)', fontSize: 13, textDecoration: 'none' }}>
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5"/><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"/><line x1="17.5" y1="6.5" x2="17.51" y2="6.5"/></svg>
                @the_scent_snob_
              </a>
